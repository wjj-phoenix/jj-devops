...
public final class TreeUtil {
    ...
    public static <Id, T extends TreeNode<Id, T>> List<T> generateTrees(List<T> nodes) {
        ...
        for (T node : nodes) {
            // 是根节点 或 父id为空 都算根节点
            if (node.root() || node.getPid() == null) {
                roots.add(node);
            } else {
                // 非根节点
                // 根据 父id 合并 子节点
                parentId2ChildrenMap.computeIfAbsent(node.getPid(), k -> new ArrayList<>()).add(node);
            }
        }
        // 如果节点列表中没有顶层根节点, 则父id 不在节点列表中 也可以算顶层节点
        if (roots.isEmpty()) {
            // 所有节点id集合
            final Set<Id> nodeIdSet = nodes.stream().map(T::getId).collect(Collectors.toSet());
            for (T node : nodes) {
                if (!nodeIdSet.contains(node.getPid())) {
                    roots.add(node);
                }
            }
        }
    ...
    private static <Id, T extends TreeNode<Id, T>> void setChildren(T parent, Map<Id, List<T>> parentId2ChildrenMap) {
        ...
        final List<T> children = parentId2ChildrenMap.getOrDefault(parentId, Collections.emptyList());
        // 避免子列表为null
    ...
    /**
     * 树节点父类，所有需要使 TreeUtil 工具类形成树形结构等操作的节点都需要实现该接口
     *
     * @param <Id> 节点id类型
     */
